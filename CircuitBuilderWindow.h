...
struct ElementParams {
    ...
    double w_slot_mm  { 2.0  };   // [mm]

    // ── ApertureWithCover ─────────────────────────────────
    double tau_cover_mm { 0.5 };  // cover gap [mm]

    // ── Cavity ───────────────────────────────────────────
    double L_cavity_mm { 100.0 }; // depth [mm]
...
private:
                   ...
                   std::function<void(double)> fn, int decimals = 6)
    {
        ...
        s->setDecimals(decimals);                    // [BUGFIX-DECIMALS-ROUNDING] FIRST
        s->setRange(mn, mx);
...
private:
    ...
    void runCompute()
    {
        ...
        const double a_g  = sp.a_mm      * 1e-3;   // [P1] mm → SI m
        const double b_g  = sp.b_h_mm    * 1e-3;   // [P1] mm → SI m
        const double t_g  = sp.t_wall_mm * 1e-3;   // [P1] mm → SI m
        ...
        for(auto* el : ordered)
        {
            ...
            case ElementType::Aperture:
                solver.addBranch(std::make_shared<AP_SlotAperture>(
                    nFrom, 0, bid,
                    a_g, b_g, ep.l_slot_mm*1e-3, ep.w_slot_mm*1e-3, t_g));
                break;
